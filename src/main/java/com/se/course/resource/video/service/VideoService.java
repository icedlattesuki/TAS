...
public class VideoService extends FileService {
    ...
    public boolean upload(HttpSession session, MultipartFile file, int courseId, String title, String profile) {
        ...
        if (isFileExist(getDirPath(courseId) + file.getOriginalFilename())) {
            int fileId = videoDAO.getFileId(getDirPath(courseId).substring(FileService.ROOT_PATH.length())+ file.getOriginalFilename());
        ...
        if (store(file, getDirPath(courseId))) {
            ...
            video.setLocation(getDirPath(courseId).substring(FileService.ROOT_PATH.length()) + file.getOriginalFilename());
    ...
    private String getDirPath(int courseId) {
        return FileService.ROOT_PATH + File.separator +"video" + File.separator + courseId + File.separator;
    }
}
